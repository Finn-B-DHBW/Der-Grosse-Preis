...
public class QuestionPanel extends JPanel {
    ...
    private Integer pointsValue; // header points
    private Question questionModel; // backing model
    ...
    // Backward-compatible no-arg constructor (no model binding)
    public QuestionPanel() {
        this(null, null);
    }

    public QuestionPanel(Integer points, Question question) {
        this.pointsValue = points;
        this.questionModel = question;

        // Grid with header + 4 rows and 3 columns using GridBagLayout
        setLayout(new GridBagLayout());
        GridBagConstraints gbc = new GridBagConstraints();
        // Tight spacing and anchoring to reduce left whitespace
        gbc.insets = new Insets(2, 0, 2, 0);
        gbc.fill = GridBagConstraints.HORIZONTAL;
        gbc.anchor = GridBagConstraints.NORTHWEST;

        // Header row spanning three columns: "<points> Punkte"
        ...
        // Configure text areas for multiline support
        for (JTextArea ta : new JTextArea[]{textAreaQuestion, textAreaAnswer1, textAreaAnswer2, textAreaAnswer3}) {
            ta.setLineWrap(true);
            ta.setWrapStyleWord(true);
        }

        // Row 1: Frage label left, textarea spans text + checkbox column
        ...
        // Place the question area only in the middle column
        gbc.gridx = 1; gbc.gridy = 1; gbc.weightx = 1.0; gbc.fill = GridBagConstraints.BOTH; gbc.gridwidth = 1;
        add(qScroll, gbc);
        // Add a label to the right of the question text area
        ...
        // Row 2: Antwort 1 label, textarea, checkbox
        ...
        // Row 3: Antwort 2 label, textarea, checkbox
        ...
        // Row 4: Antwort 3 label, textarea, checkbox
        ...
        // Visual debug: red outline around each QuestionPanel
        setBorder(BorderFactory.createLineBorder(Color.RED, 1));

        // Initialize from model (if present)
        populateFromModel();
        // Wire autosave on focus lost into Question model
        attachAutosaveListeners();
        // Bind checkboxes to model with mutual exclusivity
    ...
    // Ensure the JScrollPane shows at least two text rows of the JTextArea without clipping
    private static void enforceTwoRowViewport(JTextArea area, JScrollPane scroll) {
        if (area == null || scroll == null) return;
        // Make sure the text area is configured for 2 rows minimum
        int rows = Math.max(2, area.getRows());
        area.setRows(rows);
        // Compute a reasonable height for the given number of rows
        ...
        // Apply as minimum sizes so GridBag respects it
        Dimension taMin = new Dimension(100, contentH);
        area.setMinimumSize(taMin);
        // Let the scroll pane prefer at least that height
